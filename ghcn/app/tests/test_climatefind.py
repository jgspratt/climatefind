...
def test_compact_json_dumps():
    ...
    ret = climatefind.utils.compact_json_dumps(
        dict_for_compact_printing, indent=2, width=80
    )
    assert ret
    # print(ret)
...
def test_get_filename_hash():
    assert (
        climatefind.utils.get_filename_hash(samples[1]["filename"])
        == samples[1]["filename_hash"]
    )


def test_is_temperature_file():
    assert (
        climatefind.is_temperature_file(samples[1]["filepath"])
        == samples[1]["is_temperature_file"]
    )
    assert (
        climatefind.is_temperature_file(samples[2]["filepath"])
        == samples[2]["is_temperature_file"]
    )


def test_read_ghcn_file_meta():
    print()
    assert climatefind.read_ghcn_file_meta(samples[1]["filepath"]) == samples[1]["meta"]
    assert climatefind.read_ghcn_file_meta(samples[3]["filepath"]) == samples[3]["meta"]
    assert climatefind.read_ghcn_file_meta(samples[5]["filepath"]) == samples[5]["meta"]
    assert (
        bool(climatefind.read_ghcn_file_meta(samples[4]["filepath"]))
        == samples[4]["is_usa_location"]
    )
...
def test_scale_onto_array():
    assert (
        climatefind.scale_onto_array(
            vmin=0, vmax=365, val=1, arr=[i for i in range(0, 20)]
        )
        == 0
    )
    ...
    assert (
        climatefind.scale_onto_array(
            vmin=0, vmax=365, val=364, arr=[i for i in range(0, 20)]
        )
        == 19
    )
    ...
    assert (
        climatefind.scale_onto_array(
            vmin=0, vmax=365, val=182, arr=[i for i in range(0, 20)]
        )
        == 10
    )
...
def test_make_folium_elevation_map():
    ...
    assert climatefind.make_folium_elevation_map(
        elevation_column="aug_1_tmin", color_scheme="high_red", units="C"
    )
    assert climatefind.make_folium_elevation_map(
        elevation_column="aug_1_tmax", color_scheme="high_red", units="C"
    )
